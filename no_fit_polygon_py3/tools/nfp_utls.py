...
def nfp_rectangle(A, B):
    ...
    :return:
    """
    # Set maxA, minA bằng tọa độ của điểm đầu tiên
    min_ax = A[0]['x']
    min_ay = A[0]['y']
    max_ax = A[0]['x']
    max_ay = A[0]['y']

    # Duyệt các điểm còn lại của A để tìm max min chính xác
    for point in A[1:]:
        if point['x'] < min_ax:
            min_ax = point['x']
        if point['x'] > max_ax:
    ...
    # Set maxB, minB bằng tọa độ của điểm đầu tiên
    min_bx = B[0]['x']
    min_by = B[0]['y']
    max_bx = B[0]['x']
    max_by = B[0]['y']

    # Duyệt các điểm còn lại của B để tìm max min chính xác
    for point in B[1:]:
        if point['x'] < min_bx:
            min_bx = point['x']
        if point['x'] > max_bx:
    ...
    # Nếu kích thước của B vượt quá bin thì return
    if max_bx - min_bx > max_ax - min_ax:
        return None
    if max_by - min_by > max_ay - min_ay:
        return None

    # NPF là hình chữ nhật
    # Tọa độ bao của B được kéo về điểm minA - minB + tọa độ B[0]
    return [
        [
...
def polygon_slide_distance(A, B, direction, ignorenegative):
    ...
    for i in range(0, len(edge_b) - 1):
        ...
        for j in range(0, len(edge_a) - 1):
            ...
            if (
                (almost_equal(A1['x'], A2['x']) and almost_equal(A1['y'], A2['y']))
                or almost_equal(B1['x'], B2['x'])
                and almost_equal(B1['y'], B2['y'])
            ):
                continue
...
def segment_distance(A, B, E, F, direction):
    ...
    cross_abe = (E['y'] - A['y']) * (B['x'] - A['x']) - (E['x'] - A['x']) * (
        B['y'] - A['y']
    )
    cross_abf = (F['y'] - A['y']) * (B['x'] - A['x']) - (F['x'] - A['x']) * (
        B['y'] - A['y']
    )
